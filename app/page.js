...
function parse(wb){
 ...
 const reportRows=officerRows.map(r=>visibleIndexes.map(i=>{const v=r[i]??'';return norm(v)==='officer name'?cleanOfficerName(v):v;}));
...
function psPDF(o,rows){
 ...
 const body=rows.map((r,i)=>[i+1,r.ps,cleanOfficerName(cleanOfficerName(r.officer||o.name)),r.blo,r.supervisor,r.generated||'',r.scheduled,r.delivered,r.pending,pct(r.deliveredPct),r.docs,pct(r.docsPct),r.dates||'—',r.heldLapsed||0,r.disposal||0,pct(r.disposalPct)]);
