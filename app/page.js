...
function hearingDateColor(status){
 const s=String(status??'').toLowerCase();
 if(s.includes('pending')) return [248,105,107];
 if(s.includes('partial')) return [255,235,132];
 if(s.includes('held')) return [99,190,123];
 return null;
}
function parse(wb){
   ...
   autoTable(d,{startY:31,head:[psHeaders],body:psBody,theme:'grid',tableWidth:'wrap',
     ...
     if(data.section==='body'&&data.column.index===12){
       ...
       if(row && row.dates){
         ...
         const hc=hearingDateColor(row.status);
         if(hc) data.cell.styles.fillColor=hc;
       }
     }},
